...
if ( ! class_exists( 'TwentyTwenty_Walker_Comment' ) ) {
	...
	class TwentyTwenty_Walker_Comment extends Walker_Comment {
		...
		protected function html5_comment( $comment, $depth, $args ) {
			...
			?>
			<<?php echo $tag; // phpcs:ignore ABC_WP.Security.EscapeOutput.OutputNotEscaped -- static output ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $this->has_children ? 'parent' : '', $comment ); ?>>
						...
						<div class="comment-author vcard">
							...
							if ( 0 !== $args['avatar_size'] ) {
								if ( empty( $comment_author_url ) ) {
									...
									printf( '<a href="%s" rel="external nofollow" class="url">', $comment_author_url ); // phpcs:ignore ABC_WP.Security.EscapeOutput.OutputNotEscaped --Escaped in https://developer.wordpress.org/reference/functions/get_comment_author_url/
									echo wp_kses_post( $avatar );
								}
							}
					...
					if ( $comment_reply_link || $by_post_author ) {
						...
						<footer class="comment-footer-meta">
							...
							if ( $comment_reply_link ) {
								echo $comment_reply_link; // phpcs:ignore ABC_WP.Security.EscapeOutput.OutputNotEscaped -- Link is escaped in https://developer.wordpress.org/reference/functions/get_comment_reply_link/
							}
							if ( $by_post_author ) {
